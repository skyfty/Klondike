...
command_line_helper::command_line_helper()
    ...
    main_options.add_options()
            ...
            ("cores", po::value<uint32_t>(), "the number of cores for the solvability percentages to be run across. "
...
bool command_line_helper::parse(int argc, const char* argv[]) {
    ...
    if (vm.count("resume")) {
        ...
        resume = {0, 0, 0};
        for (uint32_t i = 0; i < cores; i++) resume.push_back(i);
    }
...
uint32_t command_line_helper::get_cores() {
    return cores;
}
